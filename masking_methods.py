...
def auto_thres_mask(option,
    ...
    print "doing blur"
    seg = sitk.DiscreteGaussian(img, variance=1)
    print "doing threshold"
    if option == "otsu":
        if not bins:
            bins = 128
        seg = sitk.OtsuThreshold(seg, insideValue=0, outsideValue=255, numberOfHistogramBins=bins)
    elif option == "huang":
        if not bins:
            bins = 128
        seg = sitk.HuangThreshold(seg, insideValue=0, outsideValue=255, numberOfHistogramBins=bins)
    elif option == "li":
        if not bins:
            bins = 256
        seg = sitk.LiThreshold(seg, insideValue=0, outsideValue=255, numberOfHistogramBins=bins)
    elif option == "triangle":
        if not bins:
            bins = 256
        seg = sitk.TriangleThreshold(seg, insideValue=0, outsideValue=255, numberOfHistogramBins=bins)
    elif option == "isodata":
        if not bins:
            bins = 256
        seg = sitk.IsoDataThreshold(seg, insideValue=0, outsideValue=255, numberOfHistogramBins=bins)
...
def binary_mask(img,
    ...
    if lt:
        ...
        clean_up(img, seg, output, "binary", dilate = dilate, tight = tight, external = external)
        return
    ...
    sitk.WriteImage(seg, os.path.join(output, "binary.nrrd"))
    print "clean up"
    seg = clean_up(img, seg, output, "binary", dilate = dilate, tight= tight, external = external)
...
def binary_mask_split(img,
    ...
    #===================================================================
    blur = sitk.DiscreteGaussian(img, variance=1)
    size = list(img.GetSize())
    z_depth = size[2]
    size[2] = int(split)
    start = [0, 0, 1]
    ...
    print "blurring"
    seg_second = sitk.DiscreteGaussian(second, variance=1)
    print "thresholding"
...
def clean_up(img,
    ...
    sitk.WriteImage(seg, os.path.join(output, name + "_binary_img.nrrd"))
...
def fill_image(npa, roll=0):
    ...
    for slice in loop:
	slice_fill = ndimage.binary_fill_holes(slice).astype(int)
	if npa_hole_filled == None:
