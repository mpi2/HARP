...
public class ROI_Importer implements PlugIn {
	...
	public void run(String arg) {
		...
		for (int i=0; i<height; i++) {
			x[i] = (int)ip.getPixelValue(0, i);
			y[i] = (int)ip.getPixelValue(1, i);
		}
		Roi roi = new PolygonRoi(x, y, height, null, Roi.POLYGON);
		Rectangle r = roi.getBoundingRect();
		ImagePlus imp = WindowManager.getCurrentImage();
		if (imp==null || imp.getWidth()<r.x+r.width || imp.getHeight()<r.y+r.height) {
			new ImagePlus(tr.getName(), new ByteProcessor(r.x+r.width+10, r.y+r.height+10)).show();
