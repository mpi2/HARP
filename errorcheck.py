...
def errorCheck(mainwindow):
    ...
    if not inputFolder :
        QtGui.QMessageBox.warning(mainwindow, 'Message', 'Warning: input directory not defined')
        mainwindow.stop = True
        return

    if not outputFolder :
        QtGui.QMessageBox.warning(mainwindow, 'Message', 'Warning: output directory not defined')
        mainwindow.stop = True
        return

    # Check if input folder exists
    if not os.path.exists(inputFolder):
        QtGui.QMessageBox.warning(mainwindow, 'Message', 'Warning: input folder does not exist')
        mainwindow.stop = True
        return

    # Check if a directory
    if not os.path.isdir(inputFolder):
        QtGui.QMessageBox.warning(mainwindow, 'Message', 'Warning: input folder is not a directory')
        mainwindow.stop = True
        return

    #Check if folder is empty
    if os.listdir(inputFolder) == []:
        QtGui.QMessageBox.warning(mainwindow, 'Message', 'Warning: input folder is empty')
        mainwindow.stop = True
        return
    ...
    if file_check:
        ...
        QtGui.QMessageBox.warning(mainwindow, 'Message', 'Warning: input folder does not contain image files')
        mainwindow.stop = True
        return
    ...
    if mainwindow.ui.checkBoxScansReconComp.isChecked():
        if mainwindow.scan_folder == "":
            QtGui.QMessageBox.warning(mainwindow, 'Message', 'Warning: Scan folder not defined')
            mainwindow.stop = True
            return
        elif not os.path.exists(mainwindow.scan_folder):
            QtGui.QMessageBox.warning(mainwindow, 'Message', "Warning: Scan folder does not exist")
            mainwindow.stop = True
            return
    ...
    if mainwindow.ui.radioButtonMan.isChecked():
        try:
            ...
            QtGui.QMessageBox.warning(mainwindow, 'Message', 'Warning: Cropping dimensions have not been defined')
            mainwindow.stop = True
            return

    # Check input directory contains something
    if os.listdir(inputFolder) == []:
        QtGui.QMessageBox.warning(mainwindow, 'Message', 'Warning: input folder is empty, please check')
        mainwindow.stop = True
        return
    ...
    while True:
        ...
        if twi0.text() == outputFolder:
            QtGui.QMessageBox.warning(mainwindow, 'Error adding job', 'Output folder is already on the processing list!')
            mainwindow.stop = True
            return
        count = count+1
    ...
    if mainwindow.ui.radioButtonUseOldCrop.isChecked() and mainwindow.ui.checkBoxCropYes.isChecked():
        ...
        if not os.path.exists(os.path.join(str(mainwindow.ui.lineEditOutput.text()),"cropped")):
            mainwindow.stop = True
            QtGui.QMessageBox.warning(mainwindow, 'Message', 'Warning: Use old crop option selected. '
    ...
    if mainwindow.ui.radioButtonuCT.isChecked() and mainwindow.ui.radioButtonDerived.isChecked():
            mainwindow.stop = True
            QtGui.QMessageBox.warning(mainwindow, 'Message', 'Warning: Derived Dimension crop option is not available for'
                                            ' uCT processing')
            return
    ...
    # Check if output folder already exists. Ask if it is ok to overwrite
        ...
        if len(os.listdir(outputFolder)) > 0:
            ...
            message = QtGui.QMessageBox.warning(mainwindow, 'Warning',
